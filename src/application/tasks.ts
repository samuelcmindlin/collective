...
export const taskSchemas = {
  task_create: z.object({ commandId, title: text(200), description: text(), ownerId: text(100), acceptance: text(), dependencies: ids.default([]) }).strict(),
  task_update: z.object({ commandId, expectedVersion, taskId: text(100), status: z.enum(['doing', 'blocked']), note: text().optional() }).strict(),
  task_submit: z.object({ commandId, expectedVersion, taskId: text(100), evidenceIds: ids.min(1), note: text() }).strict(),
  task_review: z.object({ commandId, expectedVersion, taskId: text(100), accepted: z.boolean(), note: text() }).strict(),
};
...
export class TaskCommands {
    ...
    return this.store.transaction(() => {
      ...
      return executeCommand(this.store, { principalId: agentId, commandId, name, missionId: mission.id }, { missionRevision: mission.revision, ...payload }, () => {
        switch (command.name) {
          ...
          case 'task_submit': return this.submit(agentId, mission, command.args);
          case 'task_review': return this.review(agentId, mission, command.args);
        }
      });
    });
  }
  ...
  private create(agentId: string, mission: Mission, args: z.infer<typeof taskSchemas.task_create>): Task {
    ...
    const { commandId: _commandId, ...fields } = args;
    const task = this.store.put('tasks', {
  ...
  private submit(agentId: string, mission: Mission, args: z.infer<typeof taskSchemas.task_submit>): Task {
    const task = this.currentTask(args.taskId, mission, args.expectedVersion);
    ...
    const reviewRound = (task.reviewRound ?? 0) + 1;
    const updated = this.patch(task, { status: 'review', evidence: args.evidenceIds, review: undefined, reviewRound });
    this.store.enqueue(reviewer.id, 'task', { taskId: task.id, review: true, reviewRound }, `task:${task.id}:review:${reviewRound}`);
    this.store.event('task.submitted', agentId, task.id, { note: args.note, reviewRound });
    return updated;
  }

  private review(agentId: string, mission: Mission, args: z.infer<typeof taskSchemas.task_review>): Task {
    const task = this.currentTask(args.taskId, mission, args.expectedVersion);
    ...
    this.store.event(args.accepted ? 'task.accepted' : 'task.rejected', agentId, task.id, { note: args.note });
    return updated;
  }
}
